...
const CORRECTION_RULES = [
  { pattern: /\bI want\b/gi, replace: "I’d like", reasonEs: "En contextos de servicio, ‘I’d like’ es más cortés que ‘I want’." },
  { pattern: /\bI am agree\b/gi, replace: "I agree", reasonEs: "El verbo ‘agree’ no usa ‘am’: se dice ‘I agree’." },
  { pattern: /\bmore better\b/gi, replace: "better", reasonEs: "‘Better’ ya es comparativo; no se usa ‘more better’." },
  { pattern: /\bpeoples\b/gi, replace: "people", reasonEs: "‘People’ ya es plural; no lleva ‘s’." },
  { pattern: /\bI have (\d+) years\b/gi, replace: (m,p)=>`I am ${p} years old`, reasonEs: "Edad: ‘I am … years old’." },
  { pattern: /\bI no understand\b/gi, replace: "I don’t understand", reasonEs: "Para negar en presente simple: ‘don’t’ + verbo base." },
  { pattern: /\bI am not agree\b/gi, replace: "I don’t agree", reasonEs: "Se usa ‘I don’t agree’, no ‘I am not agree’." },
];
...
export default function EnglishDialogueTutor(){
  ...
  const onSend = ()=>{
    ...
    const reply = `👍 Good!\n\n✔ Corrected version: ${correction.corrected}\n🗣 Short note (ES): ${correction.notes[0] || "Bien formulado. Continúa."}\n\n${follow}`;
  ...
  return (
    <div className={`min-h-screen w-full bg-gradient-to-b from-${BRAND.bgFrom} to-${BRAND.bgTo} text-slate-800`}>
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm md:text-base text-slate-600 mt-1">Practica conversaciones reales en inglés. Corrección automática y registro por estudiante.</p>
            </div>
          </div>
        ...
        <div className="text-center text-xs text-slate-500 mt-6">Demo — integra OpenAI en <code>onSend()</code> para correcciones IA.</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="md:col-span-2">
          ...
          <div className="flex flex-col md:flex-row gap-2 items-stretch md:items-end">
            <div className="flex-1">
              ...
              <select value={selectedStudentId} onChange={e=>setSelectedStudentId(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm bg-white">
                {students.length===0 && <option value="">— Sin estudiantes —</option>}
            ...
            <div className="flex-1">
              ...
              <input type="text" value={newStudentName} onChange={e=>setNewStudentName(e.target.value)} placeholder="Ej.: Ana Pérez" className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" />
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <button onClick={downloadTranscript} className="inline-flex items-center gap-2 rounded-2xl px-3 py-2 bg-white border border-slate-200 shadow-sm hover:shadow"><Download className="h-4 w-4" /> Exportar diálogo</button>
            <button onClick={endAndSaveSession} className="inline-flex items-center gap-2 rounded-2xl px-3 py-2 bg-amber-500 text-white shadow hover:brightness-95">Guardar sesión</button>
            <div className="ml-auto text-xs text-slate-600"><span className="mr-3">💬 Turnos: <b>{turnCount}</b></span><span>⚠️ Errores: <b>{mistakeCount}</b></span></div>
          </div>
        </div>
      </div>
      ...
      <div className="md:col-span-1">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
          <div className="flex items-center gap-2 mb-2">{lastCorrection && lastCorrection.notes.length===0 ? <CheckCircle className="h-4 w-4 text-emerald-600" /> : <XCircle className="h-4 w-4 text-amber-500" />}<span className="text-sm font-semibold">Corrección & Notas</span></div>
          {lastCorrection ? <div><p className="text-sm mb-2"><span className="font-semibold">Corrected:</span> {lastCorrection.corrected}</p><ul className="list-disc pl-5 text-sm text-slate-600 space-y-1">{lastCorrection.notes.length>0 ? lastCorrection.notes.map((n,i)=><li key={i}>{n}</li>) : <li>Bien formulado. Continúa.</li>}</ul></div> : <p className="text-sm text-slate-600">Las correcciones aparecerán aquí después de tu primer mensaje.</p>}
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard icon={<LineChart className="h-5 w-5" />} label="Turnos/ sesión" value={avgTurns} />
        <StatCard icon={<BarChart2 className="h-5 w-5" />} label="Errores/ sesión" value={avgMistakes} />
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>{students.length===0 ? <tr><td colSpan={5} className="px-2 py-4 text-center text-slate-500">Sin estudiantes.</td></tr> : students.map(s=>{ const arr = progress[s.id] || []; const tTurns = arr.reduce((a,b)=>a+b.turns,0); const tMist = arr.reduce((a,b)=>a+b.mistakes,0); return (<tr key={s.id} className="odd:bg-white even:bg-slate-50"><td className="px-2 py-2">{s.name}</td><td className="px-2 py-2">{s.course || "—"}</td><td className="px-2 py-2 text-right">{arr.length}</td><td className="px-2 py-2 text-right">{tTurns}</td><td className="px-2 py-2 text-right">{tMist}</td></tr>) })}</tbody>
          </table>
        </div>
      </div>
    </div>
